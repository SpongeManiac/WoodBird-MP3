      ...
      itemBuilder: (context) {
        Map<String, ContextItemTuple> choices = <String, ContextItemTuple>{
          'Add to queue': ContextItemTuple(
            ...
            () async {
              await widget.app.audioInterface.addToQueue([song]);
            },
          ),
          ...
          'Play single': ContextItemTuple(
            Icons.play_arrow,
            () async {
              await widget.app.audioInterface.removeAll();
              await widget.app.audioInterface.addToQueue([song]);
            },
          ),
  ...
  Future<List<AudioSource?>> create() async {
    ...
    if (result != null && result.count > 0) {
      ...
      for (var i = 0; i < result.count; i++) {
        ...
        var original = File(ogPath);
        var newPath = p.join(songsDir.path, base);
        var cacheFile = File(newPath);
        ...
        var songUri = widget.app.getSongUri(widget.app.getSongCachePath(base));
        print(songUri);
        UriAudioSource songSource = AudioSource.uri(songUri,
            tag: MediaItem(
              id: '-1',
              title: name,
              artist: '',
